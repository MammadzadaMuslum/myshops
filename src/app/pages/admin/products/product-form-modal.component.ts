        ...
        <form (ngSubmit)="onSubmit()" class="p-6 space-y-4">
          <div>
            ...
            <select [(ngModel)]="formData.shopId" name="shopId" required
                    [class.border-red-500]="errors['shopId']"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">Select a Shop</option>
              <option *ngFor="let shop of shops" [value]="shop.id">{{ shop.name }}</option>
            </select>
            <p *ngIf="errors['shopId']" class="text-xs text-red-600 mt-1">{{ errors['shopId'] }}</p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" [(ngModel)]="formData.price" name="price" required min="0" step="0.01"
                     ...
                     class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
              <p *ngIf="errors['price']" class="text-xs text-red-600 mt-1">{{ errors['price'] }}</p>
            </div>
            <div>
...
[class.border-red-500]="errors['category']"
    ...
    <app-validation-error-modal
      *ngIf="showValidationError"
      ...
      message="You cannot create a new product without filling all required fields."
      buttonText="OK"
...
export class ProductFormModalComponent implements OnInit {
  ...
  validateForm(): boolean {
    ...
    if (!this.formData.shopId) {
      this.errors['shopId'] = 'Shop selection is required';
      isValid = false;
    }
